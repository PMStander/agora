...
// ── FlipPageWrapper for react-pageflip (requires forwardRef) ──
const FlipPageWrapper = forwardRef<HTMLDivElement, {
  children: React.ReactNode;
  density?: 'hard' | 'soft';
  style?: React.CSSProperties;
}>(({ children, density = 'soft', style = {} }, ref) => (
  <div
    ref={ref}
...
FlipPageWrapper.displayName = 'FlipPageWrapper';

// ── Main BookViewer Component ──
//
// Architecture:
// 1. Render all children normally (so React components execute and Page mounts)
// 2. Use a MutationObserver + querySelectorAll to discover .book-page elements
// 3. In "slide" mode: render all children but only show the current page via CSS
// 4. In "scroll" mode: render all children in a scrollable container
// 5. In "flip" mode: render children hidden, then clone DOM nodes into react-pageflip
//
export const BookViewer: React.FC<BookViewerProps> = ({
  config,
  children,
  mode: initialMode = 'flip',
  showToolbar = true,
  showThumbnails = true,
  flipDuration = 800,
  showCover = true,
  ...
  // Discover pages from DOM after render
  useEffect(() => {
  ...
  // Apply CSS to show/hide pages in slide mode
  useEffect(() => {
    if (!renderAreaRef.current || mode === 'scroll') return;
    ...
    pages.forEach((page, i) => {
      if (mode === 'slide') {
        ...
        // In flip mode, hide the render area (we use clones)
  ...
  // Determine render area visibility
    ...
    <BookConfigContext.Provider value={config}>
      <PageCounterContext.Provider value={pageCounter}>
        <div className={`book-viewer ${className}`} style={{ textAlign: 'center', padding: '20px', ...style }}>
          ...
          {/* Render area: children are always rendered so React components execute */}
          <div
            ref={renderAreaRef}
            className="book-viewer-render-area"
            style={{
              // In slide/scroll mode: show inline, centered
              // In flip mode: render off-screen (we clone into flipbook)
              ...(isRenderVisible ? {
                display: 'flex',
                ...
                ...(mode === 'scroll' ? {
                  ...
                  // Slide mode: container sized to one page
                  width: pageWidth,
                  height: pageHeight,
                  margin: '0 auto',
                  overflow: 'hidden',
...
// ── FlipPageClone: clones a real DOM node into react-pageflip ──
