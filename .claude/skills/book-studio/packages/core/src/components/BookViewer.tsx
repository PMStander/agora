...
import type { BookViewerProps, ViewerMode } from '../types.js';
import { BookConfigContext, PageCounterContext } from './Book.js';
    ...
    <div style={{ marginBottom: '16px' }}>
      <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: showThumbnails ? '12px' : '0' }}>
        <button
          ...
          style={currentPage === 0 ? disabledStyle : buttonStyle}
        ...
        <button
          ...
          style={currentPage >= totalPages - 1 ? disabledStyle : buttonStyle}
        ...
        <div style={{ display: 'flex', gap: '2px', marginLeft: '8px' }}>
          {(['flip', 'slide', 'scroll'] as ViewerMode[]).map((m) => (
            <button
              ...
              style={mode === m ? activeButtonStyle : buttonStyle}
          ...
          {Array.from({ length: totalPages }, (_, i) => (
            <button
              ...
              style={{
                ...
                borderRadius: '4px',
                border: '1px solid',
                borderColor: i === currentPage ? '#4f46e5' : '#d1d5db',
                backgroundColor: i === currentPage ? '#4f46e5' : '#fff',
...
// ─────────────────────────────────────────
// The previous approach used flattenPages() to walk the React element tree
// looking for <Page> components by displayName. This DOES NOT WORK when
// pages are wrapped in user-defined components (e.g. <CoverPage>, <TocPage>)
// because React element tree walking can't see inside function components.
//
// Instead, we:
// 1. Always render children into a container (so all React components execute)
// 2. After render, query the DOM for .book-page elements (Page adds this class)
// 3. In slide mode: show/hide pages via CSS display property
// 4. In scroll mode: show the render area as a scrollable container
// 5. In flip mode: clone DOM nodes into react-pageflip wrappers
//
export const BookViewer: React.FC<BookViewerProps> = ({
  ...
  // Page counter for nested Page components (used by Page's registerPage)
  ...
  // Convert book dimensions to pixels (96 DPI for screen preview)
  ...
  // Load react-pageflip for flip mode (optional dependency)
  useEffect(() => {
    let cancelled = false;
    import('react-pageflip')
      .then((mod) => { if (!cancelled) setHTMLFlipBook(() => mod.default); })
      .catch(() => { if (!cancelled) setLoadError('react-pageflip not installed. Run: npm install react-pageflip'); });
    ...
    <BookConfigContext.Provider value={config}>
      <PageCounterContext.Provider value={pageCounter}>
        <div className={`book-viewer ${className}`} style={{ textAlign: 'center', padding: '20px', ...style }}>
          ...
          {/* Render area: children always render here so Page components mount to the DOM */}
          <div
            ref={renderAreaRef}
            className="book-viewer-render-area"
            style={{
              ...(isRenderVisible ? {
                display: 'flex',
          ...
          {/* Flip mode: clone pages into react-pageflip */}
          {showFlip && !loadError && HTMLFlipBook && (
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <HTMLFlipBook
                ...
                onFlip={(e: any) => handleFlip(e.data)}
                style={{}}
          ...
          {showFlip && loadError && (
            <div style={{
              width: pageWidth, height: pageHeight, margin: '0 auto',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              backgroundColor: '#fef2f2', border: '1px solid #fecaca',
              borderRadius: '8px', padding: '24px', textAlign: 'center', color: '#991b1b', fontSize: '14px',
            }}>
              <div>
                <p style={{ fontWeight: 600, marginBottom: '8px' }}>Page flip not available</p>
                <p>{loadError}</p>
              </div>
            </div>
          )}
          ...
          {showFlip && !loadError && !HTMLFlipBook && (
            <div style={{
              width: pageWidth * 2, height: pageHeight, margin: '0 auto',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              backgroundColor: '#f9fafb', borderRadius: '4px',
            }}>
